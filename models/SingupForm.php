...
class SingUpForm extends Model{
    ...
    public $message=null;
    public $suggested_password;
    public function rules(){
        return [
            [['username', 'password', 'password_repeat'],'required'],
            [['username'],'match', 'pattern' => '/^[0-9A-Za-z]{5,16}$/', 'message' => 'Must have 5-16 words'],
            [['password' , 'password_repeat'],'match','pattern' => '/^(?=.*[0-9])(?=.*[A-Z])(?=.*[a-z])[A-Za-z0-9]{7,47}$/', 'message' =>'Must contain one upercase, one letter and have 7-50 words'],
            ['username', 'userExists'],
    ...
    public function insertValues(){ 
        if($this->validate()){
            ...
            $model->access_token = Yii::$app->security->generateRandomKey();
            if ($model->save()){
                return true;
            }else{
                return false;
